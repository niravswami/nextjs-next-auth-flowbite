...
export default function SignUp({ csrfToken }) {
	...
	const validateFormValues = () => {
		...
		Object.keys(formValues)
			...
			.forEach((key) => {
				let value = ["name", "email"].includes(formValues[key].name)
				...
				if (key === "name" && value === "") {
					...
					} else if (!IS_VALID_FN.email(value)) {
						error = "invalid email address";
					}
				} else if (key === "password") {
					error = IS_VALID_FN.password(formValues.password.value);
				...
				newFormValues[key].error = error;
			});

		console.log("newFormVals", newFormValues);
		setFormValues(newFormValues);
	};
