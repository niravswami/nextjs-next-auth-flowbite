...
export default function VerifyEmail(props) {
	...
	console.log("sessionData", sessionData, "propspropsprops", props);

	const [isLinkResend, setIsLinkResend] = useState(false);
	const [resendLoading, setResendLoading] = useState(false);
	// const DEFAULT_ATERT_DATA = {
	// 	isShow: false,
	// 	boldText: "",
	...
	// 	structuredClone(DEFAULT_ATERT_DATA)
	// );
	...
	const handleVerifyLink = async () => {
		...
		let newAlertData = structuredClone(alertData);

		if (!userId || !hash || !expires) {
			newAlertData.isShow = true;
			newAlertData.boldText = "Invalid link!";
		} else {
			isLinkExpired = currentTime > expires;
			console.log({ isLinkExpired, expires: +expires, currentTime });
			if (isLinkExpired) {
				newAlertData.isShow = true;
				newAlertData.boldText = "Oops!";
			} else {
				// let response = {};
				let response = await axios({})
					.get(`${process.env.NEXT_PUBLIC_BACKEND_URL_V1}${router.asPath}`, {
						headers: {
							Authorization: `Bearer ${sessionData?.accessToken}_`,
						},
					})
					.then((resp) => resp?.data)
				...
				console.log(
					...
					response,
					"router.asPath",
					`${process.env.NEXT_PUBLIC_BACKEND_URL_V1}${router.asPath}`
				);

				if (!response?.success) {
					if (response?.status === 401) {
						return router.replace(FULL_URL.SESSION_EXPIRE);
					} else {
						newAlertData.isShow = true;
						newAlertData.boldText = "Oops!";
						newAlertData.text = response?.data?.msg || "wrong";
					}
				} else {
					await updateUserAndRedirect(response?.data?.user);
				}
			}
		}
	...
	useEffect(() => {
		...
		// if (Object.keys(router?.query).length > 0 && sessionData?.user) {
		// 	console.log("router", router);
		// 	handleVerifyLink();
		// }

		// return () => setAlertData(structuredClone(DEFAULT_ATERT_DATA));
	...
	const handleResendLink = async () => {
		...
		// console.log("handleResendLink response", response);
	};
				...
				<div className="w-full bg-white rounded-lg shadow dark:border md:mt-0 max-w-xl xl:p-0 dark:bg-gray-800 dark:border-gray-700">
					<div className="p-6 space-y-4 md:space-y-6 sm:p-8">
						{alertData?.isShow && (
							<Alert color="failure">
								<span>
									<span className="font-medium">{alertData?.boldText}</span>{" "}
									{alertData?.text}
								</span>
							</Alert>
						)}
						<div className="mb-4 text-sm text-gray-600">
							...
							your email address <strong>{sessionData?.user?.email}</strong> by
	...
	if (Object.keys(query).length > 0) {
		...
		if (!userId || !hash || !expires) {
			...
			if (isLinkExpired) {
				...
				console.log("response of link", response);
