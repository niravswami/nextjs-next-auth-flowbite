	...
	providers: [
		...
		CredentialProvider({
			...
			authorize: async (credentials, req) => {
				...
				await axios({ req })
					.post(`${process.env.NEXT_PUBLIC_BACKEND_URL_V1}/user/login`, payload)
					.then((res) => {
						user = res?.data;
					})
					.catch((err) => {
						// console.log("err", err);
						let errObj = { status: err.response.status, error: "" };
				...
				try {
					await axios({ req: {} })
						...
						.then((res) => {
							...
							isSignIn = true;
							// console.log("gt res", res);
							return res;
						})
						.catch((err) => console.log("gt catch", err));
				} catch (error) {
					console.log("try error", error);
				}
				// const isUser = await fetch(
				// 	`${process.env.NEXT_PUBLIC_BACKEND_URL_V1}/auth/github/email=${user?.access_token}`
				// );
				// const allPostsData = await isUser.json();
				// console.log("allPostsData", isUser);
			}
			// console.log("isSignInisSignIn", isSignIn);
			return isSignIn;
		},
